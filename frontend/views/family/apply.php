                ...
                <div class="row text-sm wrapper">
                    <?php $form = ActiveForm::begin([
                        ...
                        <div class="form-inline" style="margin-left: 20px; margin-right: 20px;">
                            ...
                            <a href="<?=\yii\helpers\Url::to(['family/list','Familyplayer'=>['status'=>2]])?>" class="btn btn-default">拒绝记录</a>
                            <a href="<?=\yii\helpers\Url::to(['family/list','Familyplayer'=>['status'=>3]])?>" class="btn btn-default">踢出记录</a>
                        </div>
                ...
                <div class="table-responsive">
                    <table class="table table-bordered table-hover" >
                        ...
                        <?php foreach ($data as $key => $value):?>
                            <tr>
                                ...
                                <td  class="text-center"><?=\common\models\Familyrecord::getApply_time($value['playerid'],0)?></td>
                                <td class="text-center">
                                    ...
                                    <?php elseif($value['status']==3):?>
                                        <span class="label bg-danger">已踢出</span>
                                    <?php else:?>
                                ...
                                <td  class="text-center">
                                    <?php if ($value['status']==0):?>
                                        <a onclick="return openAgency(this,'是否通过申请?')"
                                           href="<?php echo \yii\helpers\Url::to(['family/pass', 'id' => $value['id'],'status'=>1]) ?>"
                                           class="btn btn-xs btn-danger">通过</a>
                                        <a onclick="return openAgency(this,'是否通过申请?')"
                                           href="<?php echo \yii\helpers\Url::to(['family/pass', 'id' => $value['id'],'status'=>2]) ?>"
                                           class="btn btn-xs btn-danger">拒绝</a>
                                    <?php else:?>
                                        <span class="label bg-info">已审核</span>
                                    <?php endif;?>
                                </td>
                    ...
                    <?php if(empty($data)):?>
                        <div class="text-center m-t-lg clearfix wrapper-lg animated fadeInRightBig" id="galleryLoading">
                            ...
                            <h4 class="text-muted">对不起、未能找到"<?=Yii::t('app','withdraw_list')?>"相关的任何数据</h4>
                            <p class="m-t-lg"></p>
                        </div>
                    <?php endif;?>
                </div>
